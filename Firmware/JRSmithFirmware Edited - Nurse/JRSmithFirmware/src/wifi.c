...
volatile bool led_rst_flag = false;
...
{
	/* Configure PIO clock. */
	pmc_enable_periph_clk(LED_RST_ID);

	/* Adjust PIO debounce filter using a 10 Hz filter. */
	pio_set_debounce_filter(LED_RST_PIO, LED_RST_PIN_MSK, 10);

	/* Initialize PIO interrupt handler, see PIO definition in conf_board.h
	**/
	pio_handler_set(LED_RST_PIO, LED_RST_ID, LED_RST_PIN_MSK,
			LED_RST_ATTR, led_rst_handler);

	/* Enable PIO controller IRQs. */
	NVIC_EnableIRQ((IRQn_Type)LED_RST_ID);
